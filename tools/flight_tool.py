...
def parse_route(query: str) -> tuple[str | None, str | None]:
    """
    Extract departure and arrival IATA airport codes from a travel query.

    The parser supports:
    - Explicit routes such as "from Bangladesh to Japan".
    - Reversed phrasing such as "to Japan from Bangladesh".
    - IATA routes such as "DAC to NRT".
    - Origin-only queries such as "flights from Dhaka".
    - Destination-only queries such as "flights to Tokyo".
    - Travel-planning queries such as
      "Plan a 7 day Japan trip from Bangladesh".
    - Destination-only trip requests, using DEFAULT_ORIGIN_IATA.
    - Global flight queries, returning no route filters.

    Args:
        query: Natural-language travel or flight request.

    Returns:
        A tuple containing:

        - departure IATA code or None
        - arrival IATA code or None

        Examples:
            "DAC to NRT" -> ("DAC", "NRT")
            "from Bangladesh to Japan" -> ("DAC", "NRT")
            "Japan trip from Bangladesh" -> ("DAC", "NRT")
            "Japan trip" -> (DEFAULT_ORIGIN_IATA, "NRT")
            "flights from Dhaka" -> ("DAC", None)
            "global flights" -> (None, None)
    """
    q = query.strip()
    q_lower = q.lower()
    ...
    # Explicit IATA route: DAC to NRT / DAC -> NRT
    ...
    if iata_match:
        dep_iata = iata_match.group(1).upper()
        arr_iata = iata_match.group(2).upper()

        if dep_iata in AIRPORTS and arr_iata in AIRPORTS:
    ...
    if match:
        ...
        return (
            resolve_location_to_iata(origin_text),
            resolve_location_to_iata(destination_text),
        )
    ...
    # Pattern: "... Japan trip from Bangladesh"
    ...
    if match:
        ...
        # Search the rest of the query for another location,
        # which is most likely the destination.
        for mention in mentions:
            candidate_iata = resolve_location_to_iata(mention)

            if candidate_iata and candidate_iata != dep_iata:
                return dep_iata, candidate_iata

        return dep_iata, None

    # Pattern: flights/trip to X
    ...
    # Fallback: two detected locations
    if len(mentions) >= 2:
    ...
    # Destination-only travel request
    if len(mentions) == 1:
...
def search_flights(query: str, limit: int = 10) -> str:
    """
    Search AviationStack for live flight information matching a travel query.

    The function first extracts the departure and destination from the user's
    natural-language request, resolves them to IATA airport codes, and then
    queries AviationStack for matching live flights.

    Args:
        query: Natural-language travel or flight request, for example
            "Plan a Japan trip from Bangladesh" or "DAC to NRT flights".
        limit: Maximum number of flight results to return.

    Returns:
        A formatted string containing matching live flight information.

        If no flights are found, the returned string explains that no live
        flight data was available. API configuration, network, HTTP, and
        AviationStack errors are also returned as descriptive messages.
...
if __name__ == "__main__":
    print(search_flights("Plan a 7 days Japan trip from Athens"))
    print("\n" + "=" * 80 + "\n")
